...
async function fetchStore(): Promise<Store> {
  ...
  const [
    ...
    pool.query("SELECT id, slug, name_en, name_ja, prefecture_id, region_id, city, city_ja, address, address_ja, lat, lng, image_urls FROM shrines"),
    ...
    pool.query("SELECT id, shrine_id, name_en, name_ja, time_prose, time_prose_ja, start_date::text, end_date::text, origin, origin_ja, meaning, meaning_ja, ritual, ritual_ja, prayer, prayer_ja, festival_type, visitor_notes, visitor_notes_ja, sort_order FROM festivals ORDER BY sort_order"),
