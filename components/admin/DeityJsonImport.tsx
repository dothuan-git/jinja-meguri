...
import { checkDeityCompleteness } from "@/lib/admin/keyCompleteness";
...
export default function DeityJsonImport({ initialData, onSave, pending }: Props) {
  ...
  const completeness = useMemo(() => {
    ...
    try {
      return { report: checkDeityCompleteness(JSON.parse(text)), invalid: false as const };
    } catch {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder='{"name_en": "Inari Ōkami", "name_ja": "稲荷大神", "deity_type": "mythological", "titles": [], "canonical_lore": null}'
        />
      </div>
      ...
      {completeness?.report && (
        ...
        >
          <p className="font-medium">
            {completeness.report.complete
              ? `✓ All keys present — ${completeness.report.totalPresent}/${completeness.report.totalExpected}`
