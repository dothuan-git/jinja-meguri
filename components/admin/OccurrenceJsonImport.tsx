...
import { checkOccurrencesCompleteness } from "@/lib/admin/keyCompleteness";
...
export default function OccurrenceJsonImport({ onSave, pending }: Props) {
  ...
  const completeness = useMemo(() => {
    ...
    try {
      return { report: checkOccurrencesCompleteness(JSON.parse(text)), invalid: false as const };
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {parseError && <p className="rounded bg-red-50 px-3 py-2 text-sm text-red-700">{parseError}</p>}
